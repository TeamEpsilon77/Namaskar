...
class Gyalpo extends StatefulWidget {
  @override
  HomeState createState() => HomeState();
}

class HomeState extends State<Gyalpo> {
  List<Item> gyalpo = List();
  Item item;
  ...
  void initState() {
    ...
    itemRef = database.reference().child('gyalpo');
  ...
  _onEntryAdded(Event event) {
    setState(() {
      gyalpo.add(Item.fromSnapshot(event.snapshot));
    });
  }

  _onEntryChanged(Event event) {
    var old = gyalpo.singleWhere((entry) {
      return entry.key == event.snapshot.key;
    });
    setState(() {
      gyalpo[gyalpo.indexOf(old)] = Item.fromSnapshot(event.snapshot);
    });
  }
